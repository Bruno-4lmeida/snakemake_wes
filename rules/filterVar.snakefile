    ...
    shell:
        ...
        for vcf in {input}; do
            if [ -f $vcf ]; then
                ...
                # Verificando se o arquivo é de SNP ou INDEL para aplicar filtros específicos
                if [[ "$vcf" == *"raw_snps.vcf.gz"* ]]; then
                    echo "Applying SNP filters for $vcf"
                    gatk VariantFiltration -R {params.ref} -V $vcf \
                        --filter-expression "QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0" \
                        --filter-name "snp_filter" -G-filter "GQ < 20.0" -G-filter-name "lowGQ" \
                        -O $output_file &
                elif [[ "$vcf" == *"raw_indels.vcf.gz"* ]]; then
                    echo "Applying INDEL filters for $vcf"
                    gatk VariantFiltration -R {params.ref} -V $vcf \
                        --filter-expression "QD < 2.0 || FS > 200.0 || ReadPosRankSum < -20.0" \
                        --filter-name "indel_filter" -G-filter "GQ < 20.0" -G-filter-name "lowGQ" \
                        -O $output_file &
                fi
            fi
        done
